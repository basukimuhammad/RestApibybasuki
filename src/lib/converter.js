 ...
 */
export default async function convertSwaggerPathsToJsDoc(
  swaggerJsonString
) {
  try {
    ...
    for (const route in swaggerJson.paths) {
      for (const method in swaggerJson.paths[
        route
      ]) {
        const endpoint =
          swaggerJson.paths[route][method]
        jsDocLines.push('/**')
    ...
    if (error instanceof SyntaxError) {
      console.error(
        'Error parsing Swagger JSON: Invalid JSON format.'
      )
    } else {
      console.error(
        `Error processing Swagger JSON: ${error.message}`
      )
    }

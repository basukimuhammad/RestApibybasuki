    ...
    try {
      ...
      function decodeSnapAppArguments(args) {
          ...
          let decodedData = data
            ...
            .reduce(function (acc, char, index) {
              if (header.indexOf(char) !== -1) {
                return (acc +=
                  header.indexOf(char) *
                  Math.pow(encoding, index))
              }
            }, 0)
          let result = ''
          while (decodedData > 0) {
            result =
              radixArray[decodedData % radix] +
              result
            decodedData =
              (decodedData -
                (decodedData % radix)) /
        ...
        for (
          ...
          for (let j = 0; j < nonce.length; j++)
            temp = temp.replace(
              new RegExp(nonce[j], 'g'),
              j.toString()
            )

          result += String.fromCharCode(
            decodeData(temp, encoding, 10) -
              timestamp
          )
        }
      ...
      function decryptSnapSaveData(data) {
        return getDecodedSnapSaveData(
          decodeSnapAppArguments(
            getEncodedArguments(data)
          )
        )
      }
      ...
      const decryptedData =
        decryptSnapSaveData(response)
      const $ = cheerio.load(decryptedData)
      const results = []

      if (
        $('table.table').length ||
        $('article.media > figure').length
      ) {
        const thumbnailUrl = $(
        ...
        $('tbody > tr').each((_, element) => {
          ...
          let downloadUrl =
            ...
            $td
              ...
              .attr('onclick')
          const shouldRender =
            /get_progressApi/gi.test(
              downloadUrl || ''
            )
          if (shouldRender) {
            const match =
              /get_progressApi\('(.*?)'\)/.exec(
                downloadUrl || ''
              )
            downloadUrl = match
              ? match[1] || downloadUrl
        ...
        $('div.download-items__thumb').each(
          (_, thumbElement) => {
            const thumbnailUrl = $(thumbElement)
              .find('img')
              .attr('src')
            $('div.download-items__btn').each(
              (_, buttonElement) => {
                let downloadUrl = $(buttonElement)
                  .find('a')
                  .attr('href')
                if (
                  !/https?:\/\//.test(
